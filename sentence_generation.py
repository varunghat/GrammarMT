...
def load_rules(filename: str):
    """
    Load and print rules from the grammar book files, take pdf as input.
    """
    base_filename = Path(filename).stem

    rules_filename = os.path.join("extracted_rules", f"{base_filename}_sections_classified_gpt_extracted_rules_direct_parsed.json") #TODO: Fix convoluted naming system
    with open(rules_filename, "r", encoding="utf-8") as f:
        rules_data = json.load(f)

    parallel_sentences_file = os.path.join("parallel_sents", f"{base_filename}_parallel_sentences_with_pos_and_unimorph.json")
    ...
    dict_path = os.path.join("dictionary", f"{base_filename}_dictionary_cleaned.json")
    ...
    random_nouns = random.sample(list(dict.items()), no_of_random_nouns)
    random_nouns = {noun: dict[noun] for noun, _ in random_nouns}
    ...
    for sentence in tqdm(parallel_sentences_with_pos_and_unimorph[:LIMIT]):
        ...
        if noun_to_be_replaced is None:
            ...
            translation_noun_to_be_replaced = dict.get(noun_to_be_replaced, "No translation available")
        ...
        rule_text = ""
        if gender == "Masc":
            ruleset += [rule for rule in filtered_noun_rules if "masculine" in rule["description"].lower()]
            ...
            ruleset += [rule for rule in filtered_noun_rules if "feminine" in rule["description"].lower()]
            gender_ruleset_text_start = "The word to be replaced is feminine, so the following rules apply:\n"
            gender_ruleset_text = gender_ruleset_text_start + "\n".join("- " + rule["description"] for rule in ruleset)
            rule_text+=gender_ruleset_text
            #print(ruleset)
        if plural == "Plur":
            ruleset += [rule for rule in filtered_noun_rules if "plural" in rule["description"].lower()]
        ...
        if case is not None:
            if case == "Nom":
                ruleset += [rule for rule in filtered_noun_rules if "nominative" in rule["description"].lower()]
                ...
                ruleset += [rule for rule in filtered_noun_rules if "accusative" in rule["description"].lower()]
                case_ruleset_text_start = "The word to be replaced is accusative, so the following rules apply:\n"
                case_ruleset_text = case_ruleset_text_start + "\n".join("- " + rule["description"] for rule in ruleset)
                rule_text+=case_ruleset_text
                #print(ruleset)
            elif case == "Dat":
                ruleset += [rule for rule in filtered_noun_rules if "dative" in rule["description"].lower()]
                case_ruleset_text_start = "The word to be replaced is dative, so the following rules apply:\n"
                case_ruleset_text = case_ruleset_text_start + "\n".join("- " + rule["description"] for rule in ruleset)
                rule_text+=case_ruleset_text
                #print(ruleset)
            elif case == "Gen":
                ruleset += [rule for rule in filtered_noun_rules if "genitive" in rule["description"].lower()]
                case_ruleset_text_start = "The word to be replaced is genitive, so the following rules apply:\n"
                case_ruleset_text = case_ruleset_text_start + "\n".join("- " + rule["description"] for rule in ruleset)
                rule_text+=case_ruleset_text
                #print(ruleset)
            elif case == "Loc":
                ruleset += [rule for rule in filtered_noun_rules if "locative" in rule["description"].lower()]
                case_ruleset_text_start = "The word to be replaced is locative, so the following rules apply:\n"
                case_ruleset_text = case_ruleset_text_start + "\n".join("- " + rule["description"] for rule in ruleset)
                rule_text+=case_ruleset_text
                #print(ruleset)
            elif case == "Abl":
                ruleset += [rule for rule in filtered_noun_rules if "ablative" in rule["description"].lower()]
                case_ruleset_text_start = "The word to be replaced is ablative, so the following rules apply:\n"
                case_ruleset_text = case_ruleset_text_start + "\n".join("- " + rule["description"] for rule in ruleset)
                rule_text+=case_ruleset_text
                #print(ruleset)
            elif case == "Voc":
                ruleset += [rule for rule in filtered_noun_rules if "vocative" in rule["description"].lower()]
        ...
        if tense is not None:
            if tense == "Past":
                ruleset += [rule for rule in filtered_noun_rules if "past" in rule["description"].lower()]
                ...
                ruleset += [rule for rule in filtered_noun_rules if "present" in rule["description"].lower()]
                tense_ruleset_text_start = "The sentence is in present tense, so the following rules apply:\n"
                tense_ruleset_text = tense_ruleset_text_start + "\n".join("- " + rule["description"] for rule in ruleset)
                rule_text+=tense_ruleset_text
                #print(ruleset)
            elif tense == "Fut":
                ruleset += [rule for rule in filtered_noun_rules if "future" in rule["description"].lower()]
        ...
        for noun in random_nouns.keys():
            ...
            if "\n" in generated_text:
                try:
                    ...
                    print(f"Gen Sentence: {gen_sentence}"
    ...
    output_filename = os.path.join("generated_sentences", f"{base_filename}_generated_sentences.json")
